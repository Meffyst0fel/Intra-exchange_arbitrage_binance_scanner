...
# Визначення функцій для роботи з Binance API
...
def calculate_tax_rate_per_trade(trade_history, commission_rate, symbol, new_variable, first_part):
    ...
    for trade in trade_history:
        ...
        # Визначення базової та котируваної валюти
        ...
        if commission_asset != 'USDT':
            ...
            end_time = start_time + 60000  # додати хвилину для впевненості, що отримаємо потрібну свічку

            # Отримання курсів
            asset_usdt_klines = get_historical_klines(f"{commission_asset}USDT", "1m", start_time, end_time)
            if not asset_usdt_klines:
                continue
            asset_usdt_price = float(asset_usdt_klines[0][4])  # ціна закриття
            ...
            # Розраховуємо комісію для обміну
            ...
            print(f"Количество {adx_amount} {first_part} на {timestamp}:")
            print(f"Итоговая сумма после перерасчета в USDT: {usdt_amount_after_commission:.2f} USDT")
